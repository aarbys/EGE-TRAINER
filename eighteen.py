...
def start(amount_of_lines,smth_1,smth_2,smth_3,smth_4):
    ...
    print(
        'Исполнитель Робот может перемещаться по клеткам, выполняя за одно перемещение одну из двух команд: {} или {}.'.format(smth_1,smth_2))
    print('По команде {} Робот перемещается в соседнюю {} клетку, по команде {} — в соседнюю {}.'.format(smth_1,smth_3,smth_2,smth_4))
...
def example_solution(psih, type_position):
    a = [[1, 8, 8, 4], [10, 1, 1, 3], [1, 3, 12, 2], [2, 3, 5, 6]]
    if type_position == 1: a.reverse() # bottom left
    elif type_position == 2:  # bottom right
        a.reverse()
        for smth in range(len(a)): a[smth].reverse()
    ...
    if a[0][0] % psih == 0:solution_table[0][0] = a[0][0]
    else:solution_table[0][0] = 0
...
def top_left_hard_solution(table, a_o_l, corner_N, corner_L, len_N, len_L):
    ...
    sum_table_1 = 1
    for i in table: sum_table_1 += sum(i)
...
def top_right_hard_solution(table, a_o_l, corner_N, corner_L, len_N, len_L):
    ...
    def nullifier(corner_N, len_N, corner_L, len_L, sol_tab, what):
        for NUMBER in range(corner_N - len_N, corner_N):
            for LETETER in range(corner_L , corner_L + len_L):
                sol_tab[NUMBER][LETETER] = what
        return sol_tab

    sum_table = 1
    for i in table: sum_table += sum(i)
    ...
    for NUMBER in range(1, a_o_l):
        ...
        sol_tab[0][a_o_l - NUMBER - 1] = sol_tab[0][a_o_l - NUMBER] + table[0][
            a_o_l - NUMBER - 1]  # There is NUMBER like a LETTER
...
def bottom_left_hard_solution(table, a_o_l, corner_N, corner_L, len_N, len_L):
    ...
    def nullifier(corner_N, len_N, corner_L, len_L, sol_tab, what):
        for NUMBER in range(corner_N-1,corner_N + len_N-1):
            for LETETER in range(corner_L - len_L + 1, corner_L + 1):
                sol_tab[NUMBER][LETETER] = what
        return sol_tab
    ...
    for i in table:
        sum_table += sum(i)
    sol_table = []  # solution table two-dimensional massif [NUMBER][LETTER]
    ...
    sol_table = nullifier(corner_N, len_N, corner_L, len_L, sol_table, -1)

    for NUMBER in range(1,a_o_l):
        for LETTER in range(1,a_o_l):
            n_n = a_o_l - NUMBER - 1 # New number a_o_l == 11; NUMBER == 1; => n_n = 9. Like a penult massif
            l = LETTER
            if sol_table[n_n][l] != -1:
                sol_table[n_n][l] = max(sol_table[n_n][l-1],sol_table[n_n+1][l]) + table[n_n][l]
            else:
                continue
    answer.append(sol_table[0][-1])

    sol_table = nullifier(corner_N, len_N, corner_L, len_L, sol_table, sum_table)

    for NUMBER in range(1,a_o_l):
        for LETTER in range(1,a_o_l):
            n_n = a_o_l - NUMBER - 1 # New number
            l = LETTER
            if sol_table[n_n][l] != sum_table:
                sol_table[n_n][l] = min(sol_table[n_n][l-1],sol_table[n_n+1][l]) + table[n_n][l]
            else:
                continue
