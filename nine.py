...
def question():
    ...
    if type_of_problem == 1:
        c_a = temperature(type_of_problem)
    elif type_of_problem == 2:
        c_a = numbers(type_of_problem)
    return c_a
...
def gen_file(type_of_problem, random_condition):
    if type_of_problem == 1:
        ...
        sf.apply_headers_style(
            styler_obj=Styler(bg_color=utils.colors.yellow, bold=False, font=utils.fonts.calibri, font_size=12,
                              border_type=None))
        sf.apply_column_style(cols_to_style=df.columns,
                              styler_obj=Styler(bold=False, font=utils.fonts.calibri, font_size=12,
                                                border_type=utils.borders.default_grid))
        sf.apply_column_style(cols_to_style=[df.columns[0]],
                              styler_obj=Styler(bg_color=utils.colors.yellow, bold=False, font=utils.fonts.calibri,
                                                font_size=12, border_type=None))
...
def numbers_solution(random_condition, random_equality):
    ...
    if random_condition == 'jtriangle':
        ...
        for i in range(-1, 4999):
            if i == -1:
                if (first_column[0] < first_column[1] + first_column[2]) and (
                        first_column[1] < first_column[0] + first_column[2]) and (
                        first_column[2] < first_column[0] + first_column[1]):
                    count += 1
            else:
                if (table[first_column[0]][i] < table[first_column[1]][i] + table[first_column[2]][i]) and (
                        table[first_column[1]][i] < table[first_column[0]][i] + table[first_column[2]][i]) and (
                        table[first_column[2]][i] < table[first_column[0]][i] + table[first_column[1]][i]):
                    count += 1
        correct_answer = count
    elif random_condition == 'sharpangle':
        count = 0
        for i in range(-1, 4999):
            if i == -1:
                if (first_column[0] ** 2 < first_column[1] ** 2 + first_column[2] ** 2) or (
                        first_column[1] ** 2 < first_column[0] ** 2 + first_column[2] ** 2) or (
                        first_column[2] ** 2 < first_column[0] ** 2 + first_column[1] ** 2):
                    count += 1
            else:
                if (table[first_column[0]][i] ** 2 < table[first_column[1]][i] ** 2 + table[first_column[2]][
                    i] ** 2) or (
                        table[first_column[1]][i] ** 2 < table[first_column[0]][i] ** 2 + table[first_column[2]][
                    i] ** 2) or (
                        table[first_column[2]][i] ** 2 < table[first_column[0]][i] ** 2 + table[first_column[1]][
                    i] ** 2):
                    count += 1
        correct_answer = count
    elif random_condition == '90angle':
        ...
        for i in range(-1, 3199):
            if i == -1:
                first_column.sort()
                if (first_column[4] + first_column[0]) ** 2 > (
                        first_column[1] ** 2 + first_column[2] ** 2 + first_column[3] ** 2):
                    count += 1
            else:
                table_values = list()
                ...
                table_values.sort()
                if (table_values[4] + table_values[0]) ** 2 > (
                        table_values[1] ** 2 + table_values[2] ** 2 + table_values[3] ** 2):
                    count += 1
        correct_answer = count
    return correct_answer
